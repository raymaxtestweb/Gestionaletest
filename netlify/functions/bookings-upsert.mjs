...
export default async (req) => {
  try {
    ...
    const booking = {
      date,
      slot,
      status: String(body?.status || "preventivo").toLowerCase() === 'confermato' ? 'confermato' : 'preventivo',
      note: String(body?.note || "").trim(),
      occupied: body?.occupied !== false,
      source: String(body?.source || "manuale"),
      eventType: String(body?.eventType || "").trim(),
      customerName: String(body?.customerName || "").trim(),
      quoteCreatedAt: body?.quoteCreatedAt || null,
      actualTime: String(body?.actualTime || "").trim(),
    ...
    current[slot] = booking;
    bookings[date] = current;
    await store.setJSON("bookings", bookings);
    return new Response(JSON.stringify({ ok: true, booking, day: current }), { status: 200, headers: { "content-type": "application/json" } });
  } catch (error) {
